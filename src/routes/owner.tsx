...
function AdminDashboard({
  ...
  // Populate social media fields when data loads
  useEffect(() => {
    if (socialLinks) {
      setFacebook(socialLinks.facebook || '')
      setInstagram(socialLinks.instagram || '')
      setJobber(socialLinks.jobber || '')
      setGoogle(socialLinks.google || '')
      setYelp(socialLinks.yelp || '')
      setNextdoor(socialLinks.nextdoor || '')
    }
  }, [socialLinks])
  ...
  const handleSocialLinksSave = async (e: FormEvent) => {
    e.preventDefault()
    ...
    try {
      await updateSocialLinks({
        ...
        jobber: jobber || undefined,
        google: google || undefined,
        yelp: yelp || undefined,
        nextdoor: nextdoor || undefined,
      })
        ...
        {!isLoading && activeTab === 'settings' && (
          <div className="space-y-12">
            ...
            <div>
              ...
              <div className="max-w-lg bg-white p-10 rounded-3xl border border-slate-200 shadow-xl relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-blue-600 via-purple-500 to-pink-500"></div>
                <form onSubmit={handleSocialLinksSave} className="space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      type="url"
                      value={jobber}
                      onChange={(e) => setJobber(e.target.value)}
                      placeholder="https://app.getjobber.com/..."
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-sm focus:ring-2 focus:ring-blue-600 outline-none transition-all"
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="https://nextdoor.com/yourpage"
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-4 text-sm focus:ring-2 focus:ring-blue-600 outline-none transition-all"
                    />
                  </div>
                  <button
